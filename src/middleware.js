...
export async function middleware(request) {
  ...
  if (program && pathSegments.length > 2) {
    const courseSlug = pathSegments[pathSegments.length - 1]; // Last segment (e.g., "internet-of-things-training-program")
    url.pathname = `/course_details/${courseSlug}`; // Rewrite to course details path with "12" appended
